...
```mermaid
flowchart TD
    A["用户 Query<br/>故城县有什么好玩的？"]
    B["并行发起两路请求"]

    C["RAG 本地知识库<br/>(FAISS + BM25 混合检索)"]
    D["高德地图 POI API<br/>(restapi.amap.com)"]

    E["返回深度攻略<br/>(游玩时长 / 预算 / 避坑)"]
    F["返回实时 POI 列表<br/>(地址 / 评分 / 电话)"]

    G["融合层"]
    H["跨来源互补分析<br/>(检查高德 POI 名在 RAG 里有无攻略)"]
    I["LLM 生成最终回答<br/>(深度攻略 + 实时信息，双层结构)"]
